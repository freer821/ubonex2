<template>
  <div class="app-container">
    ...
    <el-table
      :data="
        main_plate_data.filter(
          data =>
            !main_plate_code_search ||
            data.main_plate_code
              .toLowerCase()
              .includes(main_plate_code_search.toLowerCase())
        )
      "
      border
      fit
    ...
    >
      <Packagelist
        :tabledata="main_plate_code_detail.data"
        table_type="main_plate_code"
        :assigned_code="main_plate_code_detail.title"
        @refresh-event="fetchTableData"
      />
    </el-drawer>
  </div>
</template>
...
export default {
  ...
  methods: {
    statusformatter(row, column) {
      row.status_code = parseInt(row.status_code);
      return row.status;
    },
    fetchTableData() {
      get_main_plate_info()
    ...
    handleMainPlateDel(row) {
      this.$confirm(
        ...
        .then(() => {
          let data = {
            action: "del_main_plate_code",
            main_plate_code: row.main_plate_code
          };
          packhouse_action(data).then(response => {
            this.$message({
              type: "success",
    ...
    handleMainPlateCodeDetailShow(row) {
      ...
      if (row.main_plate_code.trim()) {
        getPackagesInMainPlateCode(row.main_plate_code)
          .then(response => {
            this.main_plate_code_detail.title = row.main_plate_code;
            this.main_plate_code_detail.data = response.msg;
          })
          .catch(err => {
            this.$message({
              type: "error",
