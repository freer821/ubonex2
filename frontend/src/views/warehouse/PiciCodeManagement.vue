  ...
  <div class="app-container">
    ...
    <el-table
      :data="
        pici_data.filter(
          data =>
            !pici_code_search || data.pici_code === parseInt(pici_code_search)
        )
      "
      border
      fit
    ...
    >
      <Packagelist
        :tabledata="pici_code_detail.data"
        table_type="pici_code"
        :assigned_code="pici_code_detail.title"
...
export default {
  ...
  methods: {
    ...
    handlePiciCodeDetailShow(row) {
      ...
      if (row.pici_code) {
        getPackagesInPiciCode(row.pici_code)
          .then(response => {
            this.pici_code_detail.title = row.pici_code.toString();
            this.pici_code_detail.data = response.msg;
          })
          .catch(err => {
            this.$message({
              type: "error",
